...
{"questions":[{"category":"카테고리명","question":"질문 내용","intent":"검증하려는 포인트"}]}`;
  ...
  <script>window.onload=function(){window.print();}<\/script>
    ...
    // /api/* 호출 시 현재 로그인 세션을 함께 보낸다.
    // 서버가 이 토큰으로 호출자를 확인하므로 반드시 붙여야 한다 — 없으면 요청이 401로 거부된다.
    ...
    function renderAuditLog() {
      ...
      tbody.innerHTML = auditData.map(r =>
        `<tr><td class="text-sm text-gray">${escHtml(r.time)}</td><td>${escHtml(r.user)}</td><td>${escHtml(r.action)}</td><td>${escHtml(r.target)}</td><td>${escHtml(r.ip)}</td></tr>`
      ).join('');
    }
